...
public class IndexedSheet<K: Comparable & Hashable>: AtomicSheet {
  ...
  public func update(values: String..., for key: K, offset: Int, binarySearch: Bool = true) async throws {
  ...
  public func place(row: [String], binarySearch: Bool = true) throws {
    try self.placeUnsafe(s: self, row: row, binarySearch: binarySearch)
  }
  ...
  public func delete(row: [String], binarySearch: Bool = true) throws {
    try delete(rowWithKey: indexer(row), binarySearch: binarySearch)
  }

  public func delete(rowWithKey key: K, binarySearch: Bool = true) throws {
    try deleteUnsafe(rowWithKey: key, binarySearch: binarySearch)
  }
  ...
  func position(for key: K, in slice: ArraySlice<[String]>, binarySearch: Bool)
...
//extension IndexedSheet {
//
//  public func synchronize(with rowFunction: @escaping () async -> [[String]]) async throws -> Int {
//      let function = await rowFunction()
//      function.map { rows -> Int in
...
//                  let correctIndex = self.position(
//                    for: index, in: self.data.suffix(from: 1), binarySearch: false)
